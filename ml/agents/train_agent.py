...
def _train_one(symbol: str, tf: str, h: int, feats: pd.DataFrame, labels_long: pd.DataFrame,
    ...
    # Encode classes consistently
    classes = np.array(["buy", "sell", "none"], dtype=object)
    # Compute class weights to mitigate imbalance
    # Use built-in balanced weighting to avoid class-name mapping issues
